...
const RegisterCommandChannelsSectionComponent = ({ guildId, isEditMode = false }: RegisterCommandChannelsSectionProps) => {
  ...
  const registerCommandChannels = displaySettings?.register_command_channels || [];
  ...
  const isChannelSelected = (channelId: string): boolean => {
    return registerCommandChannels.some(ch => ch.id === channelId);
  };
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader>
          <CardTitle>Register Command Channels</CardTitle>
          <CardDescription>
            Select channels where the /register command can be used. Leave empty to use bot command channels or allow in all channels.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          {channelsLoading ? (
            ...
            <>
                          ...
                          {categoryChannels.map((channel) => {
                            ...
                            return (
                              <div key={channel.id} className="flex items-center space-x-2">
                                <Checkbox
                                  id={`register-channel-${channel.id}`}
                                  checked={isSelected}
                                ...
                                />
                                <Label
                                  htmlFor={`register-channel-${channel.id}`}
              ...
              {registerCommandChannels.length === 0 ? (
                <p className="text-sm text-muted-foreground italic">
                  No channels selected - will use bot command channels or allow in all channels
                </p>
              ) : (
                <div className="space-y-2">
                  <Label className="text-sm text-muted-foreground">Selected channels:</Label>
                  <div className="flex flex-wrap gap-2">
                    {registerCommandChannels.map((channel) => (
                      <div
                        key={channel.id}
...
export const RegisterCommandChannelsSection = memo(RegisterCommandChannelsSectionComponent);
RegisterCommandChannelsSection.displayName = 'RegisterCommandChannelsSection';
